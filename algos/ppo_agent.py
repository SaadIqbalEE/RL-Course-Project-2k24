...
class PPOAgent(BaseAgent):
    ...
    def get_action(self, observation, evaluation=False):
        x = torch.from_numpy(observation).float().unsqueeze(0).to(self.device) #.unsqueeze(0)
        action_dist, _ = self.policy.forward(x)
        if evaluation:
            action = action_dist.mean
            log_prob = None  # Log probability isn't needed in evaluation
            #action = action_dist.probs.argmax()
        else:
            action = action_dist.sample()
            log_prob = action_dist.log_prob(action)  # Compute log probability
            
        if len(action.shape) == 0:  # Ensure action is not scalar
            action = action.unsqueeze(0)

        # Ensure action is squeezed and converted to numpy for the environment
